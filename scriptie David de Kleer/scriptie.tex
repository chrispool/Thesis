...
\ttt{feature dictionaries} en labels. De classifier die we voor categorieen gebruiken 
...
\s{Experimenten, resultaten en evaluatie}
...
We hebben dus voor zowel train- als testset kappa-scores van 0.8 behaald. Volgens \citeauthor{manning2008introduction} is 
...
zorgt ook voor wat onduidelijkheid en zou in de toekomst eventueel weggelaten kunnen worden. Ook in de categorie \ttt{entertainment}
...
We hebben in de DEVTEST een gemiddelde accuracy van 89\% behaald, waarvoor we 10 keer cross-validation hebben toegepast. We hebben gekozen om \it{accuracy} als
...
\vl
Bij de TEST (\ttt{./ClassifierCreator -test}) hebben we met 3 verschillende classifiers getest: \it{Naive Bayes}, \it{Maximum Entropy} en
...
van 81\% komt. De \ttt{wordOverlap}-features dragen veel minder bij: de accuracy ligt daarmee nog zo rond de 60\%. De \ttt{location}-feature maakt
...
de hand om ons systeem met hun systeem te vergelijken. Het lijkt erop dat zij alleen cijfers presenteren voor events die \ttt{real-world
...
\ss{Visualisatie en verrijking van events}

Een beschrijving van de resultaten van ons systeem zou natuurlijk niet compleet zijn zonder de visualisaties van \ttt{vis/map/} toe te 
voegen. Nadat de \ttt{EventDetective} (\ref{EventDetective}) de door de \ttt{ClassifierCreator} gegenereerde classifiers heeft 
...
\begin{figure}[H]
  \centering
    \image[width=0.6\textwidth]{standaardvis.png}
